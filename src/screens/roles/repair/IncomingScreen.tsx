...
export default function IncomingScreen({ navigation }: any) {
  ...
  const handleStartRepair = async (asset: Asset) => {
    Alert.alert(
      'Start Repair',
      `Accept asset ${asset.assetNumber} and start repair cycle?`,
      [
        ...
        { 
          ...
          onPress: async () => {
            ...
            try {
              // Note: hardcoded repairCategoryId for UI simplicity, in reality this should be selected
              await RepairRepository.startRepair({
                assetId: asset.id,
                shopId: assignedLocationId || 'UNKNOWN',
                repairCategoryId: 'ROH', // Default for demonstration
                userId: employeeId || 'UNKNOWN',
              });
              
              Alert.alert('Success', 'Repair started successfully (Offline).');
              loadIncomingAssets();
            } catch (error: any) {
        ...
        <ScrollView style={styles.list}>
          {assets.length === 0 ? (
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptyText}>No incoming assets.</Text>
            </View>
          ) : (
            assets.map(asset => (
              <View key={asset.id} style={styles.assetCard}>
                ...
                <TouchableOpacity 
                  ...
                  onPress={() => handleStartRepair(asset)}
...
const styles = StyleSheet.create({
  ...
  actionBtn: { backgroundColor: '#0A74DA', paddingHorizontal: 16, paddingVertical: 12, borderRadius: 8, minWidth: 80, alignItems: 'center' },
