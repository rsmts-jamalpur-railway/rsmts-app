...
export default function MfgDashboard({ navigation }: any) {
  const { employeeId, assignedLocationId, can } = useAuth();

  return (
    <ScrollView style={styles.container}>
      <Text style={styles.greeting}>GOOD MORNING, {employeeId || 'EMP'}</Text>
      <Text style={styles.sectionTitle}>[ MANUFACTURING - {assignedLocationId || 'UNKNOWN'} ]</Text>
      ...
      <View style={styles.actionGrid}>
        {can('mfg:start') && (
          <TouchableOpacity style={styles.actionBtn} onPress={() => navigation.navigate('Orders')}>
            <Icon name="factory" size={24} color="#FFFFFF" />
