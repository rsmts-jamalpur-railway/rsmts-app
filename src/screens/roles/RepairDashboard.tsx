...
export default function RepairDashboard({ navigation }: any) {
  const { employeeId, assignedLocationId, can } = useAuth();

  return (
    <ScrollView style={styles.container}>
      <Text style={styles.greeting}>GOOD MORNING, {employeeId || 'EMP'}</Text>
      <Text style={styles.sectionTitle}>[ REPAIR OPERATIONS - {assignedLocationId || 'UNKNOWN'} ]</Text>
      ...
      <View style={styles.actionGrid}>
        {can('repair:start') && (
          <TouchableOpacity style={styles.actionBtn} onPress={() => navigation.navigate('Incoming')}>
            <Icon name="inbox-arrow-down" size={24} color="#FFFFFF" />
            <Text style={styles.actionText}>INCOMING</Text>
          </TouchableOpacity>
        )}
        
        {can('repair:start') && (
          <TouchableOpacity style={styles.actionBtn} onPress={() => navigation.navigate('ActiveRepairs')}>
        ...
        {can('repair:resume') && (
          <TouchableOpacity style={styles.actionBtn} onPress={() => navigation.navigate('OnHold')}>
            <Icon name="pause-circle-outline" size={24} color="#FFFFFF" />
            <Text style={styles.actionText}>ON HOLD</Text>
          </TouchableOpacity>
        )}
        
        <TouchableOpacity style={styles.actionBtn} onPress={() => navigation.navigate('PendingQA')}>
          <Icon name="shield-check-outline" size={24} color="#FFFFFF" />
          <Text style={styles.actionText}>PENDING QA</Text>
        </TouchableOpacity>
      </View>

      <Text style={styles.sectionTitle}>TAT ALERTS</Text>
      <View style={styles.emptyState}>
        <Text style={styles.emptyText}>TAT data unavailable (Read API Missing)</Text>
      </View>
    </ScrollView>
  );
}
